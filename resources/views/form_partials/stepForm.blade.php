...
<div class="p-form-card__group">
    <label for="time" class="p-form-card__label">{{ __('Time') }}</label>

    <div class="p-form-card__input-area">
        <input id="time" type="number" class="c-input p-form-card__time-input @error('time') c-is-invalid @enderror"
               name="time" value="@if(!empty(old('time'))){{old('time')}}@elseif(!empty($step->time)){{$step->time}}@else @endif" placeholder="10000時間以内(半角数字)"
               required min="1" max="10000" autocomplete="time">
...
<label for="child_step" class="p-form-card__label">{{ __('STEP(max:10)') }}</label>
@error('child_step.*.*')
    <p class="c-invalid-feedback" role="alert">
        <strong>{{ $message }}</strong>
    </p>
