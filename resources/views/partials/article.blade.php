...
<div class="p-step-panel c-panel">
    ...
    <!-- パネル全体をリンクにする -->
    <a href="{{route('articles.show', $article->id)}}" class="p-step-panel__link-large" title="チャレンジページ"></a>

    <div class="p-step-panel__stepImg-area">
        <img src="@if(empty($article->article_img)){{asset('/img/no_img_article.jpg')}}
        @else{{$article->article_img}}@endif" class="p-step-panel__stepImg" alt="STEPの画像">
    </div>
    ...
    <div class="p-step-panel__contents">
        ...
        <div class="p-step-panel__title-area l-flexbox">
            <h3 class="p-step-panel__title">{{Str::limit($article->title,53)}}</h3>
        </div>
...
{{--        </div>--}}
        ...
        <div class="p-step-panel__bottom l-flexbox">
            ...
            <div class="@if( Route::currentRouteName() === 'mypage.post') p-step-panel__myStep-challenge-area @else u-ml-auto @endif
            ...
            @if(Route::currentRouteName() === 'mypage.post')
                <form action="{{ route('article.delete', $article->id) }}" method="POST" class="p-step-panel__deleteForm">
                    @csrf
                ...
                <a class="c-btn c-btn--blue c-btn--mystep" href="{{route('mypage.myarticleEdit', $article->id)}}">{{__('Edit')}}</a>
            @endif
